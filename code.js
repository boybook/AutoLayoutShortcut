            ...
            if (isFrameLike(node)) {
                for (let child of node.children) {
                    if (isFrameLike(child)) {
                        ...
                        const nodeText = child;
                        Promise.all(nodeText.getRangeAllFontNames(0, nodeText.characters.length)
                            .map(figma.loadFontAsync)).then(() => {
                            setTimeout(() => {
...
function layoutAlignPrimaryFill(node) {
    node.layoutAlign = 'STRETCH';
    node.primaryAxisSizingMode = 'FIXED';
}
function layoutAlignPrimaryHug(node) {
    node.layoutAlign = 'INHERIT';
    node.primaryAxisSizingMode = 'AUTO';
}
function layoutAlignCounterFill(node) {
    node.layoutGrow = 1;
    node.counterAxisSizingMode = 'FIXED';
}
function layoutAlignCounterHug(node) {
    node.layoutGrow = 0;
